...
class TestViews(TestCase):
    ...
    def test_sushi_post_valid_form_existing_order_and_drink(self):
        ...
        order.drink = Drink.objects.create()
        order.save()
        response = self.client.post(reverse('sushi_order'), {"nigiri_sushi": 5, "inari_sushi": 2, "maki_sushi": 1, "temaki_sushi":2, "soy_oil": 1, "wasabi":1})
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse("ramen_order"))
        order.refresh_from_db()
        self.assertEqual(order.sushi, Sushi.objects.get(nigiri_sushi = 5, inari_sushi= 2, maki_sushi=1, temaki_sushi = 2, soy_oil= 1, wasabi= 1)) 
    ...
    def test_drink_post_valid_form_existing_order_and_sushi(self):
        ...
        order.sushi = Sushi.objects.create()
        order.save()
        response = self.client.post(reverse('drink_order'), {"sake": 4, "beer": 1, "choya": 2, "green_tea": 2, "water": 1})
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse("ramen_order"))
        order.refresh_from_db()
        self.assertEqual(order.drink, Drink.objects.get(sake = 4, beer = 1, choya = 2, green_tea = 2, water = 1)) 
